...
The Update (Week 12)
===
For week 12 it the assignment was to include k-fold crossvalidation which I have done previous week at [The Data] under __Data Preparation__ (Subsetting). 
Also some kind of classification or clustering was to be executed, k-means clustering I have performed in the first or second week in [The Data] under __Data Preparation (week 12 update)__ (Dimensionality Reduction).  
Classification I have done previous week as well and is included in [Modeling], some confusion matrices can be found there as well.

As for what I have done this week:  
I have performed further analysis on the principal components (as was requested by many who have reviewed my portfolio so far). This can be found in [The Data] under __Data Preparation (week 12 update)__ (Dimensionality Reduction) as well as under __Further analysis on the Principal Components (week 12 update)__.
...
### __Data Preparation (week 12 update)__ (Dimensionality Reduction)<br/>Not all data is useful for a classifier, therefore we need to reduce the dimensionality of our dataset. {data-commentary-width=750}
```{r PCA & Clusters, fig.width=10, fig.align='center'}
load('data/scaled_track_features.RData')
...
As you can see, the data is now seperated into two columns over the first principal component. Each column seems to have a cluster at the bottom but there is still plenty of spread in each column.  
...
If the data can be effectively clustered we would see 7 separate clusters, and in each cluster there will be only one type of points.
However, as you can see in _Figure 2_ and _Figure 3_ (can be found interactive in the next two stories) most data points are all in one big cluster and the outlines of each cluster all cross each other. To further illustrate this I show below that cluster 1 contains songs of each artist.

We do see that in both plots there are some clear outliers, especially in the principal component plot they are very prominent. I will further analyse this two stories further in _further analysis on the Principal Components.
...
### __Figure 4__ (interactive)
```{r}
plotly_build(pca_plt)
```

### __Further analysis on the Principal Components (week 12 update)__<br/>I further analyze the principal component via a 3d plot on the first 3 components with k-means clustering applied.{data-commentary-width=750}
```{r}
#all_songs_vari <- varimax(all_songs_pca$x)
#ggplot(all_songs_vari, aes)
```
```{r}
ply_pca <- plot_ly() %>% 
  add_trace(
...
#### Next week I will create delta and delta deltas of the timbre and or tempo to use for my models.
...
train_data <- apply(all_songs[,1:11], 2, scale_01)     # Scale track features data
```
...
#### Here I will create a support vector machine on the principal components.

### __Keras model on Principal Components__
#### Here I will create a deep learning keras model on the principal components.
