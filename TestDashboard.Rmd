...
Modeling (Week 11 Update) {.storyboard}
===
...
* A model trained on deltas of timbre/chroma and delta delta of timbre/chrome.
...
### __SVM on Track Features__<br/>I create a support vector machine model on the Track Features data and assess its performance. {data-commentary-width=600}
...
This data includes the track features without the _mode_ feature scaled to be in the range [0,1] and seperated into 5 parts.
...
I have printed the accuracy of each model together with its confusion matrix.
...
```{r Build Model Function}
build_model <- function() {
  model <- keras_model_sequential() %>%
    layer_dense(units=25, activation='relu', input_shape = 9) %>% 
...
```{r Keras Model (K-Fold CV), eval=FALSE}
num_epochs <- 150
...
```
```{r, eval=FALSE}
accuracy_history_plot <- ggplot(average_accuracy_history, aes(x=epoch, y=validation_accuracy)) + 
  geom_line() +
  theme_minimal() +
  labs(title="Mean accuracy per epoch")
ggplotly(accuracy_history_plot)
```
```{r, eval=FALSE}
...
```{r, eval=FALSE}
load("data/average_accuracy_history.RData")
max(average_accuracy_history$validation_accuracy)
argmax(average_accuracy_history, rows=FALSE)
```
...
```{r Load Dynamic Time Warps, include=FALSE, eval=FALSE}
load('plots/lf_fs_ld.RData')
load('plots/lf_ko_ld.RData')
```
```{r Plot Dynamic Time Warps, eval=FALSE}
db_st_plt
dbst_dt_plt
```
