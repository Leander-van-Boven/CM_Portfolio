...
To give a good insight in these features and get a quick overview of which of these may be show some clear differences between the artists, I have combined all songs from all artists into one dataset. Previous week I plotted each feature in a seperate plot (they can be found [here](https://github.com/Saltpile123/CM_Assignment_Week_6/blob/Week_6/README.md)). From these plots I immediately could observe that the _Mode_ features was not useful. Some other features didn't seem to show any clear patterns on their own too. That is why I am going to plot some features against each other with the colors of the data-points corresponding to the artist. If we are able to destinguish clusters we may be able to train a classifier on only these features.
...
First of all we need to look whether there are any outliers in our dataset, since they will upset the classifier thus lowering its performance. However removing data that is just outside the cluster of songs of a certain artist will lower the diversity and can cause the classifier to overfit more easily.
...
As mentioned before most of the other features didn't show much patterns too, however since we can't rule out that they might be useful we still need to include this data is some way. This, however, may mean that we get alot of information-sparse data. 
